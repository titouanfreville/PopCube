...
func TestChannelStore(t *testing.T) {
	...
	Convey("Testing Getters", t, func() {
		...
		csi.Save(&channel0, ds)
		csi.Save(&channel1, ds)
		// csi.Update(&channel1, &channel1New, ds)
		csi.Save(&channel2, ds)
		csi.Save(&channel3, ds)
	...
	Convey("Testing delete channel", t, func() {
		...
		Convey("Deleting a known channel should work", func() {
			appError := csi.Delete(&channel2, ds)
			So(appError, ShouldBeNil)
			So(appError, ShouldNotResemble, dberror)
			So(csi.GetByName("God", ds), ShouldResemble, &Channel{})
		})
		...
		// 		appError := csi.Delete(&channel3, ds)
		// 		So(appError, ShouldNotBeNil)
		// 		So(appError, ShouldNotResemble, dberror)
		...
		// 		So(csi.GetAll(ds), ShouldResemble, &channelList1)
