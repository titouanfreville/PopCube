...
type Organisation struct {
	OrganisationId   uint64 `gorm:"primary_key;column:idOrganisation;AUTO_INCREMENT" json:"-"`
...
func (organisation *Organisation) isValid() *AppError {
	...
	if len(organisation.OrganisationName) == 0 || utf8.RuneCountInString(organisation.OrganisationName) > ORGANISATION_DISPLAY_NAME_MAX_RUNES {
		return NewLocAppError("Organisation.IsValid", "model.organisation.is_valid.organisation_name.app_error", nil, "id="+strconv.FormatUint(organisation.OrganisationId, 10))
	}

	if !IsValidOrganisationIdentifier(organisation.OrganisationName) {
		return NewLocAppError("Organisation.IsValid", "model.organisation.is_valid.not_alphanum_organisation_name.app_error", nil, "id="+strconv.FormatUint(organisation.OrganisationId, 10))
	}

	if utf8.RuneCountInString(organisation.Description) > ORGANISATION_DESCRIPTION_MAX_RUNES {
		return NewLocAppError("Organisation.IsValid", "model.organisation.is_valid.description.app_error", nil, "id="+strconv.FormatUint(organisation.OrganisationId, 10))
	}
